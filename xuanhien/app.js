...
function runPreview() {
  ...
  const students = dataRows.map(row => {
    ...
    row.forEach((cellVal, i) => {
      ...
      if (key === "skip") {
        ...
        const fallbackKey = "col_" + slugify(headerRow[i]);
        FIELD_LABELS[fallbackKey] = headerRow[i];
        fields[fallbackKey] = val;
      }
    });
  ...
  const unmapped = headerRow.filter((h, i) => h && !mappedKeys[i]);
  let note = `Tìm thấy ${students.length} dòng học sinh hợp lệ.`;
  if (unmapped.length) {
    note += ` Các cột không tự nhận diện được sẽ được giữ nguyên tên: ${unmapped.join(", ")}.`;
  }
